...
class IssueProvider (model.Provider):
    ...
    def create (self, record):
        ...
        with self.get_db_cursor() as cur:
            cur.execute("INSERT INTO issue (uuid, status, url, image_url, site, lang, title, article_type, body, body_html, body_markdown, description, description_html, timestamp_creation, timestamp_publish) VALUES(%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)", (record["uuid"], 1, record["url"], record["image_url"], record["site"], record["lang"], record["title"], 'Issues', record["body"], record["body_html"], record["body_markdown"], record["description"], record["description_html"], record["timestamp_creation"], record["timestamp_publish"],))
            return Issue(record)
    def update (self, record, request):
        ...
        with self.get_db_cursor() as cur:
            ...
            cur.execute("UPDATE issue SET (status, url, image_url, site, lang, title, body, body_html, body_markdown, description, description_html, timestamp_publish) = (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s) WHERE uuid=%s", (record.status, record.url, record.image_url, record.site, record.lang, record.title, record.body, record.body_html, record.body_markdown, record.description, record.description_html, record.timestamp_publish, record.uuid,))
            return True
